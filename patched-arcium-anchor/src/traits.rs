...
use anchor_lang::{prelude::Pubkey, solana_program::account_info::AccountInfo, Result};
...
pub trait InitCompDefAccs<'info> {
    ...
    /// Returns the pre-computed weight (compute units) for this circuit.
    /// This value is read from the `.weight` file generated during circuit compilation.
    fn weight(&self) -> u64;
}
